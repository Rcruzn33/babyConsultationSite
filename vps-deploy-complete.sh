...
scp simple-deployment.html root@31.97.99.104:/var/www/index.html

# If scp fails, use alternative method
if [ $? -ne 0 ]; then
    echo "SCP failed, using alternative deployment method..."
    
    # Create the deployment via SSH
    ssh root@31.97.99.104 << 'REMOTE_COMMANDS'
    
    # Create the exact replica HTML file
    cat > /var/www/index.html << 'HTML_EOF'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
HTML_EOF
    ...
    echo "🌐 Your website is now live at: http://31.97.99.104"
    echo "🎨 Complete exact replica with two-column layout and hero image"
...
echo "📱 Exact replica of original design deployed"
