        ...
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center">
            <div className="flex-shrink-0">
              <Link href="/">
                <h1 className="text-2xl font-bold text-baby-blue cursor-pointer hover:opacity-80 transition-opacity">
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navigation.map((item) => (
                <Link key={item.name} href={item.href}>
                  <a
                    className={`px-3 py-2 text-sm font-medium transition-colors hover:text-baby-blue ${
                      isActive(item.href)
                        ? "text-soft-dark"
                        : "text-medium-gray"
                    }`}
        ...
        <div className="md:hidden bg-white border-t">
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navigation.map((item) => (
              <Link key={item.name} href={item.href}>
                <a
                  className="block px-3 py-2 font-medium hover:text-baby-blue transition-colors"
